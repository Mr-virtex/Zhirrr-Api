...
var creatorList = ['@Hazn Ramdhan','@Hazn','@Mr-virtex'];
...
loghandler = {
    notparam: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'parameter apikey tidak di temukan masukan paramater apikey!!!',
        getApikey: 'apikey ? Chat me https://wa.me/+6281539336835 Free'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'masukan parameter api key'
    },
    noturl: {
        status: false,
        creator: `${creator}`,
    ...
    nottext2: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'masukan parameter ke text2'
    },
    notnabi: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'hazn') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'hazn') return res.json(loghandler.invalidKey)
...
router.get('/randomanime', async (req, res, next) => {
    ...
    if(!apikeyInput) return res.json(loghandler.notparam)
    if (apikeyInput != 'hanz') return res.json(loghandler.invalidKey)
    Nimel()
    .then(hasil => {
    	fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=120&key=761ea2d5575581057a799d14e9c78e28&image=${hasil}&name=Hazn`))
    .then(response => response.json())
    .then(data => {
